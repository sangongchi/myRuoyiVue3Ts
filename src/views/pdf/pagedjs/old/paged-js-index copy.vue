...
async function generatePDFPreview() {
  ...
  try {
    ...
    if (cached) {
      ...
      renderCachedPDF(cached.originalHtml || cached.html)
      return
    }
...
async function renderDirectly(contentElement: Element, cacheKey: string, originalHtml: string) {
  ...
  try {
    ...
    if (container) {
      ...
      await pdfCacheManager.set(cacheKey, container.innerHTML, originalHtml, pagedjsStyles, flow.total)
    }
...
function renderCachedPDF(html: string) {
  const container = document.getElementById('pdfCon')
  if (container) {
    ...
    // ✅ 3. 设置 HTML 内容（使用原始 HTML，让浏览器重新渲染）
    container.innerHTML = html

    // ✅ 4. 强制触发浏览器重排 (Reflow)，确保 DOM 立即渲染
    void container.offsetHeight

    // ✅ 5. 等待下一个 tick，确保 DOM 完全渲染
    nextTick(() => {
      // ✅ 6. 标记渲染完成状态
