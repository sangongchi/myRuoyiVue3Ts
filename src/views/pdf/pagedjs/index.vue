...
const imageUrl = ref(testImage) // 长图url
const imagePages = ref<Array<{ offset: number; height: number }>>([])
const pagedJsInstance = ref<any>(null)
...
async function generatePDFPreview() {
  ...
  try {
    ...
    await renderDirectly(contentElement)
  } catch (error: any) {
...
// paged js 转换pdf形式预览html
async function renderDirectly(contentElement: Element) {
  pagedJsInstance.value = new Paged.Previewer()
  const cssBlob = new Blob([pagedjsStyles], { type: 'text/css' })
  const cssUrl = URL.createObjectURL(cssBlob)

  try {
    const flow = await pagedJsInstance.value.preview(contentElement, [cssUrl], document.getElementById('pdfCon'))
